...
app.get('/:id' , (req , res)=> {
    ...
    let obj = {} ;
    data.forEach(book => {
        if(book.id === parseInt(req.params.id))obj = book ;
    })
    if (Object.keys(obj).length != 0)
        res.send(obj) ;
    else res.status(404).send("The id you requested wasn't found") ;
...
app.delete('/book/:id', (req, res) => {
        ...
        const bookId = parseInt(req.params.id);
        if (bookId == undefined)res.send("please send book id") ;
...
app.get('/bookByName/:bookName' , (req , res)=> {
    let arrOfBooks: Array<object> = [] ;
    let name = req.params.bookName.toString().trim().toLowerCase() ;
    console.log(`name ${name}`) ;
    data.forEach(book => {
        if (book.title.toLowerCase().trim() === name) arrOfBooks.push(book) ;
    })
    if (arrOfBooks.length == 0)res.status(404).send("Sorry no books by that name")
    res.send( arrOfBooks );
})
app.get('/bookByYear/:bookYear' , (req , res)=> {
    ...
    data.forEach(book => {
        if (book.publicationYear === year) arrOfBooks.push(book) ;
    })
    if (arrOfBooks.length == 0)res.status(404).send("Sorry no books published that year")
    res.send( arrOfBooks );
})
